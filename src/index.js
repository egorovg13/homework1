...
function findAllPSiblings(where) {
    ...
    for (let child of where.children) {
        if (child.nextElementSibling === null) {
            break;
        } else if (child.nextElementSibling.tagName === 'P') {
            pSiblings.push(child);
        }
    }
...
function deleteTextNodes(where) {
    ...
    const textDestroyer = node => {
        if (node.nodeType == 3) {
            node.remove();
        }
    };
  
    where.childNodes.forEach(textDestroyer);
...
function deleteTextNodesRecursive(where) {
    let recTxtDestr = node => {
        node.nodeType === 3 ? node.textContent = '' : deleteTextNodesRecursive(node);
    };

    where.childNodes.forEach(recTxtDestr);
}
...
const counterFun = node => {
  if (node.nodeType === 3) {
    resultObj.texts++
  }
}

root.childNodes.forEach(counterFun);
}
